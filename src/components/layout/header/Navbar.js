...
const Navbar = ({ isActiveMobileMenu, setIsActiveMobileMenu, isSticky }) => {
	...
	return (
		<nav>
			<ul
				className={`nav flex items-center gap-x-5  xl:gap-30px  ${
					headerType === 4
						? `2xl:gap-x-35px  lg:border lg:border-primary-color lg:px-10 rounded-full`
						: ` 2xl:gap-x-45px`
				}`}
			>
				{navItems?.length
					? navItems?.map(({ name, path, path2 }, idx) => (
							<li key={idx} className="nav_item group relative hidden lg:block">
								<Link
									...
									} capitalize py-10px relative z-0  ${
										headerType === 4
